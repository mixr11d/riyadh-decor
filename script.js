...
(function () {
  ...
  // =========================================================================
  // 1. إعدادات حساب قوقل إعلانات وبيانات التواصل
  // =========================================================================
  const GOOGLE_ADS_CONFIG = {
    CONVERSION_ID: 'AW-18403659658',
    LABELS: {
      CALL: 'emj2CPPe8-UcEIqfxsdE',     // لابل الاتصال الهاتفي
      WHATSAPP: 'q9vlCPbe8-UcEIqfxsdE',   // لابل الواتساب
      FORM: 'IEKICLmm6OUcEIqfxsdE'      // لابل النموذج
    }
  };
  ...
  // =========================================================================
  // 2. التثبيت التلقائي لكود قوقل إعلانات في جميع الصفحات برمجياً
  // =========================================================================
  function autoInjectGoogleTag() {
    ...
    if (!document.querySelector(`script[src*="${GOOGLE_ADS_CONFIG.CONVERSION_ID}"]`)) {
      ...
      console.log('⚡ تم تفعيل Google Ads Tag تلقائياً على هذه الصفحة.');
    }
  }

  // تنفيذ الحقن التلقائي فوراً
  autoInjectGoogleTag();
  ...
  function sendConversion(type) {
    try {
      const label = GOOGLE_ADS_CONFIG.LABELS[type];
      if (!label) return;

      const sendTo = `${GOOGLE_ADS_CONFIG.CONVERSION_ID}/${label}`;
      ...
      console.log(`🚀 جاري إرسال إحالة [${type}] إلى قوقل...`);
    } catch (e) {
      console.warn('Tracking bypassed:', e);
    }
  }
  ...
  // =========================================================================
  // 5. إدارة النماذج الذكية وإرسال إحالة الـ Form
  // =========================================================================
  function initGlobalFormTracker() {
    document.addEventListener('submit', function (e) {
      const form = e.target;
      if (!form || !form.classList.contains('smart-lead-form')) return;
      ...
      // 🎯 إرسال إحالة النموذج لقوقل إعلانات
      sendConversion('FORM');

      // تجهيز رسالة الواتساب وفتحها
      const whatsappMsg = 
  ...
  // =========================================================================
  // 6. حقن أزرار الاتصال والواتساب العائمة تلقائياً في كل الصفحات
  // =========================================================================
  function injectFloatingActionButtons() {
    if (document.querySelector('.floating-actions-right')) return;
